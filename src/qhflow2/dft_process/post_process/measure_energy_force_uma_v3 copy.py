...
_basis = "def2svp"
_tol = 1e-8          # Tolerance for density matrix calculation
_use_gpu = False     # Use GPU for PySCF calculations
...
def set_calculation_settings(pred_xc="b3lyp", gt_xc="b3lyp", calc_xc="pbe", basis="def2svp", tol=1e-8, use_gpu=False):
    """Set global calculation settings for worker processes"""
    global _pred_xc, _gt_xc, _calc_xc, _basis, _tol, _use_gpu
    _pred_xc = pred_xc
    _gt_xc = gt_xc
    _calc_xc = calc_xc
    _basis = basis
...
def _prepare_measure_directory(base_model_path: str, start_frac: float, end_frac: float, indices: List[int] = None, tol: float = 1e-8) -> Tuple[str, str]:
    """Create measurement directory based on requested range or index list"""
    tol_str = f"{tol:.0e}".replace("-", "n").replace("+", "")
    if indices:
        dir_name = f"measure_{_format_indices_indicator(indices)}_tol{tol_str}"
    elif _is_full_fraction_range(start_frac, end_frac):
        dir_name = f"measure_full_tol{tol_str}"
    else:
        indicator = f"{_format_fraction_label(start_frac)}_{_format_fraction_label(end_frac)}"
        dir_name = f"measure_{indicator}_tol{tol_str}"
...
def init_pyscf_optimized(atoms, pos, unit="ang", xc="pbe", basis="def2svp", grid_level=None, grid_response=None):
    ...
    if _use_gpu:
        try:
            # Ensure gpu4pyscf is available
            import gpu4pyscf
            calc_mf = calc_mf.to_gpu()
        except ImportError:
            logger.warning("gpu4pyscf not installed. Falling back to CPU.")
    ...
    settings = {
        ...
        "basis": calc_mf.basis,
        "unit": unit,
        "use_gpu": _use_gpu and hasattr(calc_mf, 'with_gpu') and calc_mf.with_gpu
    }
...
def init_worker(pred_xc, gt_xc, calc_xc, basis, tol, use_gpu):
...
def compute_energy_forces_from_hamiltonian(hamiltonian, atoms, calc_overlap, calc_mf, grad_frame, convention=None, tol=1e-8):
    ...
    with Timer("    Calc DM"):
        density, res = calc_dm0_from_ham_(atoms, overlap, ham_transformed, tol=tol)
    ...
    with Timer("    Force calc"):
        forces = -grad_frame.kernel(mo_energy=mo_energy, mo_coeff=-mo_coeff, mo_occ=mo_occ)
...
def process_single_file(args: Tuple) -> Dict:
    ...
    tol_str = f"{_tol:.0e}".replace("-", "n").replace("+", "")
    pred_processed_suffix = f"_processed_{_pred_xc}_tol{tol_str}"
    ...
    try:
        ...
        with Timer("PRED processing"):
            ...
            if has_pred_calc_forces and has_pred_mo and has_pred_ham:
                ...
                _, _, pred_mo_energy, pred_ham_transformed = compute_energy_forces_from_hamiltonian(
                    pred_data["pred_hamiltonian"], atoms, calc_overlap, calc_mf, grad_frame, convention, tol=_tol
                )
                ...
                pred_energy, pred_forces, pred_mo_energy, pred_ham_transformed = compute_energy_forces_from_hamiltonian(
                    pred_data["pred_hamiltonian"], atoms, calc_overlap, calc_mf, grad_frame, convention, tol=_tol
                )
                pred_data.update({
            ...
            if os.path.exists(gt_calc_path):
                ...
                if has_gt_energy and has_gt_forces and has_gt_mo and has_gt_ham:
                    ...
                    _, _, gt_mo_energy, gt_ham_transformed = compute_energy_forces_from_hamiltonian(
                        gt_data["hamiltonian"], atoms, calc_overlap, calc_mf, grad_frame, convention, tol=_tol
                    )
...
def process_all_files(file_list, pred_xc, gt_xc, calc_xc, basis, num_workers=4, chunksize=1, maxtasksperchild=1000, output_path=None, tol=1e-8, use_gpu=False):
    ...
    with ctx.Pool(
        ...
        initargs=(pred_xc, gt_xc, calc_xc, basis, tol, use_gpu),
...
def pandas_summary(results, output_path, postfix, keys, unit="meV"):
    ...
    for key in keys:
        stats_data.append({
            "metric": key,
            "mean": results[key].mean() * conversion_factor,
            "std": results[key].std() * conversion_factor,
            "median": results[key].median() * conversion_factor,
            "min": results[key].min() * conversion_factor,
            "max": results[key].max() * conversion_factor,
            "unit": unit
        })
    ...
    with open(md_path, 'w') as f:
        ...
        f.write(f"|--------|------|-----|--------|-----|-----|\n")
        for item in stats_data:
            f.write(f"| {item['metric']} | {item['mean']:.6f} | {item['std']:.6f} | "
                   f"{item['median']:.6f} | {item['min']:.6f} | {item['max']:.6f} |\n")
    ...
    with open(txt_path, 'w') as f:
        ...
        for item in stats_data:
            ...
            f.write(f"  Mean   : {item['mean']:>12.6f} {unit}\n")
            f.write(f"  Std    : {item['std']:>12.6f} {unit}\n")
            f.write(f"  Median : {item['median']:>12.6f} {unit}\n")
            f.write(f"  Min    : {item['min']:>12.6f} {unit}\n")
            f.write(f"  Max    : {item['max']:>12.6f} {unit}\n\n")
...
def main(
    ...
    set_calculation_settings(pred_xc=pred_xc, gt_xc=gt_xc, calc_xc=calc_xc, basis=basis, tol=tol, use_gpu=use_gpu)
    ...
    tol_str = f"{tol:.0e}".replace("-", "n").replace("+", "")
    pred_processed_suffix = f"_processed_{pred_xc}_tol{tol_str}"
    logger.info(f"  pred_processed:    {pred_processed_suffix} (using {pred_xc.upper()}, tol={tol})")
    ...
    measure_dir_path, measure_dir_name = _prepare_measure_directory(
        model_path_cls.model_path, start_frac, end_frac, indices=indices_for_dir, tol=tol
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--calc_xc", type=str, default="b3lyp",
                       help="XC functional for Calc reference (default: pbe)")
    ...
    parser.add_argument("--tol", type=float, default=1e-3,
                       help="Tolerance for density matrix calculation")
    parser.add_argument("--gpu", action="store_true",
                       help="Enable GPU acceleration for PySCF calculations (requires gpu4pyscf)")
