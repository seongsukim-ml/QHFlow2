...
class eSCNMDBackbone_ham(nn.Module, MOLEInterface):
    ...
    def forward(self, data_dict: AtomicData, ham_features: list[torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        node_feats_H = torch.zeros_like(node_feats_S).to(data_dict["pos"].device)
        ...
        time_message = torch.zeros_like(x_message[:, 0, :]).to(data_dict["pos"].device)
        ...
        for i in range(self.num_layers):
            ...
            if i == self.num_layers - 1:
                x_message_mid = x_message

        # Final layer norm
        x_message = self.norm(x_message)

        xy_pair_message = None
        for i in range(len(self.xy_blocks)):
            with record_function(f"xy message passing {i}"):
                xy_pair_message_res = self.xy_blocks[i](
                    x_message_mid,
                    x_edge,
                    graph_dict["edge_distance"],
                    graph_dict["edge_index"],
            ...
            if xy_pair_message is None:
                ...
                xy_pair_message = xy_pair_message + xy_pair_message_res

        xy_pair_message = self.xy_norm(xy_pair_message)
