...
@registry.register_model("escnmd_backbone_uma_ham")
class eSCNMDBackbone_uma_ham(nn.Module, MOLEInterface):
    def __init__(
        self,
    ...
    def forward(
        ...
        ham_features: list[torch.Tensor],
        x_message_input: torch.Tensor | None,
        ...
        with record_function("atom embedding"):
            ...
            x_message_original = self.sphere_embedding(data_dict["atomic_numbers"]).unsqueeze(1)
            if x_message_input is not None:
                x_message = x_message + x_message_input
            x_message[:, 0, :] = x_message[:, 0, :] + x_message_original[:, 0, :]
