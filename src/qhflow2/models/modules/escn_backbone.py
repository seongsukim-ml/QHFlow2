...
class eSCNMDBackbone_ham(nn.Module, MOLEInterface):
    ...
    def forward(self, data_dict: AtomicData, ham_features: list[torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        matrix_mix_list = [x_message_original[:,0,:], node_feats_H[data_dict["batch"]]]
        if self.use_block_H:
            matrix_mix_list.append(node_feats_H_init[data_dict["batch"]])
        if self.use_block_S:
            matrix_mix_list.append(node_feats_S[data_dict["batch"]])
        if self.use_time_embedding:
