...
class NodeNodeInteraction(torch.nn.Module):
    def __init__(
        self,
        sphere_channels: int,
        m_hidden_channels: int,
        ...
        self.m_hidden_channels = m_hidden_channels
        ...
        self.so2_linear = SO2_Linear(
            self.sphere_channels,
            self.m_hidden_channels,
            self.lmax,
            self.mmax,
            self.mappingReduced,
        ...
        self.scalar_mlp = nn.Sequential(
            nn.Linear(self.sphere_channels, self.m_hidden_channels * self.lmax, bias=False),
            nn.SiLU()
        )
        self.act = GateActivation(
            lmax=self.lmax,
            mmax=self.mmax,
            num_channels=self.m_hidden_channels,
            m_prime=True,
        )
        self.so2_linear_2 = SO2_Linear(
            self.m_hidden_channels,
    ...
    def forward(self, x,edge):
        ...
        x = self.so2_linear(x,edge)
        x = self.act(gating_scalars, x)
        x_mid = self.so2_linear_2(x, edge)
        if self.m_hidden_channels == self.m_output_channels and self.residual:
            x = x + x_mid
        return x
    ...
    def forward(
        ...
        for idx in range(len(edge_index_partitions)):
            ...
            if len(new_embeddings) > 8:
                ...
                if return_x_message:
                    x_messages = [torch.stack(x_messages).sum(axis=0)]
        if return_x_message:
            return torch.stack(new_embeddings).sum(axis=0), torch.stack(x_messages).sum(axis=0)
        else:
...
class Edgewise_xy(torch.nn.Module):
    def __init__(
        ...
        # for edge_channel in self.edge_channels_list:
        #     self.len_edge_channels += edge_channel[0]
...
class Edgewise_xy3(torch.nn.Module):
    def __init__(
        self,
        sphere_channels: int,
        hidden_channels: int,
        ...
        self.fc1 = nn.Sequential(
            nn.Linear(3 * self.sphere_channels, self.hidden_channels),
            nn.SiLU(),
            nn.Linear(self.hidden_channels, self.lmax + 1),
        )
        ...
        for idx, m_size in enumerate(self.m_split_sizes):
            for i in range(m_size):
                self.scale_mapping.append(idx)
                
        self.scale_mapping = torch.tensor(self.scale_mapping)        
    ...
    def forward_chunk(
        ...
        self.scale_mapping = self.scale_mapping.to(x_message.device)
        with record_function("SO2Conv"):
            # Rotate the irreps to align with the edge
            x_message = torch.bmm(wigner_and_M_mapping, x_message)
            ...
            # SO2 convolution
            x_message, x_0_gating = self.so2_conv_1(x_message, x_edge)
            scale_1 = self.fc1(x_s0).view(-1, self.lmax + 1, 1)[:, self.scale_mapping, :]
            x_message = x_message * scale_1

            # M-prime...
            x_message = self.act(x_0_gating, x_message)

            x_message = self.so2_conv_2(x_message, x_edge)
            scale_2 = self.fc2(x_s0).view(-1, self.lmax + 1, 1)[:, self.scale_mapping, :]
            x_message = x_message * scale_2
...
class eSCNMD_Block_xy(torch.nn.Module):
    def __init__(
        self,
        sphere_channels: int,
        hidden_channels: int,
...
class eSCNMD_Block_xy3(torch.nn.Module):
    def __init__(
        self,
        sphere_channels: int,
        hidden_channels: int,
