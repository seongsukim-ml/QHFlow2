...
from qhflow2.common.matrix_transforms import matrix_transform_blocks, transform_coefficient_blocks


# PySCF — lazy loaded via base_module._ensure_pyscf()
# Only needed for inference/DFT validation, not training
...
from qhflow2.common.matrix_transforms import _get_orbital_mask, compute_chc_from_blocks, compute_chc_full_from_blocks, _reconstruct_full_matrix, cut_orbital_coefficients, _cut_matrix_3d
...
class LitModel_flow(LitModel):
    ...
    def _predict_step(self, batch, batch_idx, log_outputs=True):
        """Prediction step that saves outputs in dft-dataset standard format.

        Saves predictions using dft-dataset's PredictionCollector (NPZ format).
        Legacy .pt format is also saved for backward compatibility.
        At the end of test epoch, _on_predict_epoch_end() finalizes and saves.
        """
        batch = self.post_processing(batch, self.default_type)
        batch_one = batch.clone()
        self.cur_batch_size = len(batch)

        # Setup output dirs (legacy .pt format)
        pred_dir = None
        gt_dir = None
        ...
        # Init prediction collector (once)
        if not hasattr(self, '_pred_collector'):
            self._pred_collector = _init_prediction_collector(self)
            self._gt_hamiltonians = []
        ...
        if self.qh9:
            ...
            has_gt = self._batch_has_ground_truth_hamiltonian(batch_one)
            gt_hamiltonian = None
            if has_gt:
            ...
            for i in range(self.cur_batch_size):
                H_pred_np = sample["hamiltonian"][i].detach().cpu().numpy()
                S_np = gt_overlap[i].detach().cpu().numpy()
                atoms_np = batch_one[i].atoms.cpu().numpy().flatten()
                pos_np = batch_one[i].pos.cpu().numpy()

                # Collect for dft-dataset format
                self._pred_collector["hamiltonians"].append(H_pred_np)
                self._pred_collector["overlaps"].append(S_np)
                self._pred_collector["atomic_numbers"].append(atoms_np)
                self._pred_collector["positions"].append(pos_np)

                if has_gt and gt_hamiltonian is not None:
                    self._gt_hamiltonians.append(gt_hamiltonian[i].detach().cpu().numpy())
                ...
                if pred_dir is not None:
                    torch.save({"pred_hamiltonian": sample["hamiltonian"][i].cpu(), **config},
                               pred_dir / f"pred_{file_index}.pt")
                if has_gt and gt_dir is not None:
                    torch.save({"hamiltonian": gt_hamiltonian[i].cpu(), **config},
                               gt_dir / f"gt_{file_index}.pt")
        else:
            for i in range(self.cur_batch_size):
                H_pred = sample["hamiltonian"][i].detach().cpu()
                overlap = batch_one[i]["overlap"].squeeze(0).cpu()
                atoms = batch_one[i].atoms.squeeze(1).cpu()
                pos = batch_one[i].pos.cpu()

                self._pred_collector["hamiltonians"].append(H_pred.numpy())
                self._pred_collector["overlaps"].append(overlap.numpy())
                self._pred_collector["atomic_numbers"].append(atoms.numpy().flatten())
                self._pred_collector["positions"].append(pos.numpy())

                if self._batch_has_ground_truth_hamiltonian(batch_one):
                    self._gt_hamiltonians.append(batch_one[i].hamiltonian.squeeze(0).cpu().numpy())

                # Legacy .pt save
                file_index = self._next_pred_file_index()
                if pred_dir is not None:
                    torch.save({
                        "pred_hamiltonian": H_pred, "overlap": overlap,
                        "pos": pos, "atoms": atoms,
                        "format": "e3nn", "length_unit": "bohr",
                    }, pred_dir / f"pred_{file_index}.pt")
                if self._batch_has_ground_truth_hamiltonian(batch_one) and gt_dir is not None:
                    gt = {
                        "hamiltonian": batch_one[i].hamiltonian.squeeze(0).cpu(),
                        "overlap": overlap, "pos": pos, "atoms": atoms,
                        "format": "e3nn", "length_unit": "bohr",
                    }
        ...
        if self._batch_has_ground_truth_hamiltonian(batch_one):
            ...
            for key in metrics.keys():
                self.log(
                    f"pred/sample_{key}", metrics[key],
                    on_step=True, on_epoch=True,
                    prog_bar=True if key == "loss" else False,
                    sync_dist=True, batch_size=self.cur_batch_size,
                )
        return None

    def on_test_epoch_end(self):
        """Finalize and save predictions in dft-dataset format."""
        super_fn = getattr(super(), "on_test_epoch_end", None)
        if super_fn is not None:
            super_fn()

        if not hasattr(self, '_pred_collector') or not self._pred_collector["hamiltonians"]:
            return
        ...
        _save_prediction_result(self, output_dir)
...
# ── Prediction I/O helpers (dft-dataset integration) ─────────────


def _init_prediction_collector(lit_model) -> dict:
    """Initialize prediction collection buffers."""
    return {
        "hamiltonians": [],
        "overlaps": [],
        "atomic_numbers": [],
...
def _save_prediction_result(lit_model, output_dir):
    """Save collected predictions as dft-dataset PredictionResult.

    Saves:
      - predictions.npz — PredictionResult with hamiltonian matrices
    ...
    gt_hams = lit_model._gt_hamiltonians
    n = len(collector["hamiltonians"])

    logger.info(f"Saving {n} predictions in dft-dataset format...")

    # Build PredictionResult
    convention = "pyscf" if lit_model.qh9 else "e3nn"
    basis = "def2-svp" if lit_model.qh9 else "unknown"

    result = PredictionResult(
        matrices={"hamiltonian": collector["hamiltonians"]},
        atomic_numbers=collector["atomic_numbers"],
        positions=collector["positions"],
        convention=convention,
        metadata={
            "model": lit_model.model.__class__.__name__,
            "dataset": "QH9" if lit_model.qh9 else "MD17",
            "basis": basis,
            "convention": convention,
            "n_ode_steps": lit_model.num_ode_steps_test,
        },
    )
    ...
    logger.info(f"  {result.describe()}")

    # Compute comparison if GT available
    if gt_hams:
        logger.info("  Computing comparison metrics...")
        try:
            report = result.compare(
                ref_matrices={"hamiltonian": gt_hams},
            )
            report_path = output_dir / "comparison_report.txt"
            with open(report_path, "w") as f:
                f.write(repr(report))
            logger.info(f"  Comparison report saved: {report_path}")
            if report.matrix_errors and "hamiltonian" in report.matrix_errors:
                h_err = report.matrix_errors["hamiltonian"]
                logger.info(f"  Hamiltonian MAE: {h_err['mae']:.6f} Ha")
                logger.info(f"  Hamiltonian RMSE: {h_err['rmse']:.6f} Ha")
                logger.info(f"  Hamiltonian max: {h_err['max']:.6f} Ha")
        except Exception as e:
            logger.warning(f"  Comparison failed: {e}")

    # Per-molecule property extraction (eigenvalues, gap)
    if collector["overlaps"]:
        logger.info("  Extracting orbital properties...")
        try:
            homo_errors, lumo_errors, gap_errors = [], [], []
            for i in range(min(n, 500)):  # cap at 500 for speed
                H_pred = collector["hamiltonians"][i]
                S = collector["overlaps"][i]
                Z = collector["atomic_numbers"][i]
                n_electrons = int(sum(Z))

                props_pred = properties_from_hamiltonian(H_pred, S, n_electrons)

                if gt_hams and i < len(gt_hams):
                    props_gt = properties_from_hamiltonian(gt_hams[i], S, n_electrons)
                    homo_errors.append(abs(props_pred["homo"] - props_gt["homo"]) * 27211.386)  # meV
                    lumo_errors.append(abs(props_pred["lumo"] - props_gt["lumo"]) * 27211.386)
                    gap_errors.append(abs(props_pred["gap_eV"] - props_gt["gap_eV"]) * 1000)  # meV

            if gap_errors:
                logger.info(f"  HOMO MAE: {np.mean(homo_errors):.1f} meV")
                logger.info(f"  LUMO MAE: {np.mean(lumo_errors):.1f} meV")
                logger.info(f"  Gap MAE:  {np.mean(gap_errors):.1f} meV")

                # Save property metrics
                prop_path = output_dir / "property_metrics.npz"
                np.savez_compressed(str(prop_path),
                    homo_errors=np.array(homo_errors),
                    lumo_errors=np.array(lumo_errors),
                    gap_errors=np.array(gap_errors),
                )
                logger.info(f"  Property metrics saved: {prop_path}")
        except Exception as e:
            logger.warning(f"  Property extraction failed: {e}")

    # Cleanup
    del lit_model._pred_collector
    del lit_model._gt_hamiltonians