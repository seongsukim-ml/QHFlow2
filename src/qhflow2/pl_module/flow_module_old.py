        ...
        try:
            for key in keys:
                if key == "waloss":
                    ...
                    target_energy = self.build_final_matrix(
                        target,
                        target.diagonal_energy,
                        target.non_diagonal_energy,
                        transform=True,
                        out_torch=True,
                    )

                    target_orb = self.build_final_matrix(
                        target,
                        target.diagonal_orbital_coefficients,
                        target.non_diagonal_orbital_coefficients,
                        transform=True,
                        out_torch=True,
                    )
    ...
    def sample_qh9(
        ...
        if self.qh9:
            res_outputs = {
                ...
                "hamiltonian_non_diagonal_blocks": outputs["hamiltonian_non_diagonal_blocks"] / self.non_diagonal_hamiltonian_scale,
            }
            if self.use_full_target:
