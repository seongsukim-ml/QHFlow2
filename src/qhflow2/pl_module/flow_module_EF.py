...
# pyscf — lazy loaded via base_module._ensure_pyscf()


from qhflow2.pl_module.base_module import LitModel, convention_dict, build_fock_matrix
from torch_geometric.data import Batch
from qhflow2.utils import AOData, WDs, WDs_batch, Expansion, get_total_cycles
from e3nn import o3
import os

from qhflow2.models.EFS_head import MLP_EFS_head, MLP_EFS_head_ref, MLP_EFS_head_ref_approx, MLP_EFS_head_interact2
from qhflow2.models.EFS_head import MLP_EFS_head2, MLP_EFS_head_interact3, MLP_EFS_head_interact4
# ==========================================
...
class LitModel_flow_EF(LitModel):
    """
    ...
    def __init__(self, conf):
        ...
        self.freeze_backbone = conf.get("freeze_backbone", False)
        ...
        # EFS Head parameters
        self.direct_forces = conf.get("direct_forces", True)
        self.regress_stress = conf.get("regress_stress", False)
        self.regress_forces = conf.get("regress_forces", True)
        self.reduce = conf.get("reduce", "sum")

        self.efs_head_type = conf.get("efs_head_type", None)
        self.use_efs_head = True

        if self.efs_head_type is None:
            self.use_efs_head = False
            self.efs_head = None
        elif self.efs_head_type.lower() in map(lambda x: x.lower(), ["MLP", "MLP_interact2", "MLP_2", "MLP_interact3", "MLP_interact4"]):
            logger.info(f"Using MLP_EFS_head")
            if self.efs_head_type.lower() == "MLP_interact2".lower():
                self.efs_head = MLP_EFS_head_interact2(
                    self.model,
                    reduce=self.reduce,
                    direct_forces=self.direct_forces,
                    regress_stress=self.regress_stress,
                    regress_forces=self.regress_forces,
                    use_gt_H=conf.get("use_gt_H", True),
                )
            elif self.efs_head_type.lower() == "MLP_2".lower():
                self.efs_head = MLP_EFS_head2(
                    self.model,
                    reduce=self.reduce,
                    direct_forces=self.direct_forces,
                    regress_stress=self.regress_stress,
            ...
            elif self.efs_head_type.lower() == "MLP_interact3".lower():
                self.efs_head = MLP_EFS_head_interact3(
                    self.model,
                    reduce=self.reduce,
                    direct_forces=self.direct_forces,
                    regress_stress=self.regress_stress,
                    regress_forces=self.regress_forces,
                    use_atomrefs=conf.get("use_atomrefs", False),
                )
            elif self.efs_head_type.lower() == "MLP_interact4".lower():
                self.efs_head = MLP_EFS_head_interact4(
                    self.model,
                    reduce=self.reduce,
                    direct_forces=self.direct_forces,
                    regress_stress=self.regress_stress,
        ...
        elif self.efs_head_type.lower() == "MLP_ref".lower() or self.efs_head_type.lower() == "MLP_ref_approx".lower():
            self.unit = conf.get("unit", "ang")
            self.basis = conf.get("basis", "def2svp")
            self.xc = conf.get("xc", "b3lyp")
            
            logger.info(f"Using {self.efs_head_type} with unit: {self.unit}, basis: {self.basis}, xc: {self.xc}")
            
            if self.efs_head_type.lower() == "MLP_ref".lower():
                self.efs_head = MLP_EFS_head_ref(
                    self.model,
                    reduce=self.reduce,
                    direct_forces=self.direct_forces,
                    regress_stress=self.regress_stress,
            ...
            elif self.efs_head_type.lower() == "MLP_ref_approx".lower():
                self.efs_head = MLP_EFS_head_ref_approx(
                    self.model,
                    reduce=self.reduce,
                    direct_forces=self.direct_forces,
                    regress_stress=self.regress_stress,
            ...
            self.efs_head.build_final_matrix = self.build_final_matrix
            self.efs_head.cal_orbital_and_energies = self.cal_orbital_and_energies
        else:
            logger.error(f"Unknown efs_head_type: {self.efs_head_type}")
            raise ValueError(f"Unknown efs_head_type: {self.efs_head_type}")


        ## Dual target mode
        self.use_full_target = conf.get("use_full_target", False)
        if self.use_full_target:
            logger.info(f"Using full / residue target mode")
        else:
            logger.info(f"Using single target mode")
    ...
    def _corrupt_qh9(self, batch, batch_t):
        ...
        if self.use_full_target:
            target_full_ham = batch["diagonal_hamiltonian"]
            batch.init_full_ham_t = (
                random_ham * (1 - batch_t_reshape) + target_full_ham * batch_t_reshape
            )
        return batch
        ...
        try:
            for key in keys:
                if key == "waloss":
                    ...
                    # gt_ham = self.build_final_matrix(
                    #     target,
                    ...
                    # Select the orbital index along the h_dim
                    # Ex h_dim [32,24,18 ... ] shape of orbital_energies [sum(h_dim)]
                    target_energy = torch.split(target.orbital_energies, target.h_dim.tolist())
                    target_orb = []
                    for i in range(len(target.orbital_coefficients)):
                        target_orb.append(torch.tensor(target.orbital_coefficients[i], dtype=out_ham[0].dtype).cuda())
                    ...
                    # out_ham_nested = torch.nested.as_nested_tensor(gt_ham)
                    ...
                    # res = []
                    # for i in range(len(num_orbs)):
                    #     pred = torch.einsum('ji, jk, kl -> il ', target_orb[i], out_ham_nested[i], target_orb[i])
                    #     diff = pred.diag() - target_energy[i]
                    #     diff[num_orbs[i] + 1 :] = diff[num_orbs[i] + 1 :] * 0.01
                    #     res.append(diff)
                    ...
                    diff_diagonal = (
                        outputs[f"{key}_diagonal_blocks"] - target[f"diagonal_{target_key}"]
                    )
                    mse_diagonal = torch.sum(
                        diff_diagonal**2 * target[f"diagonal_{target_key}_mask"], dim=[1, 2]
                    )
                    mae_diagonal = torch.sum(
                        torch.abs(diff_diagonal) * target[f"diagonal_{target_key}_mask"],
                        dim=[1, 2],
                    )
                    count_sum_diagonal = torch.sum(
                        target[f"diagonal_{target_key}_mask"], dim=[1, 2]
                    )
                    ...
                    diff_non_diagonal = (
                        ...
                        - target[f"non_diagonal_{target_key}"]
                    )
                    mse_non_diagonal = torch.sum(
                        diff_non_diagonal**2 * target[f"non_diagonal_{target_key}_mask"],
                        dim=[1, 2],
                    )
                    mae_non_diagonal = torch.sum(
                        torch.abs(diff_non_diagonal) * target[f"non_diagonal_{target_key}_mask"],
                        dim=[1, 2],
                    )
                    count_sum_non_diagonal = torch.sum(
                        target[f"non_diagonal_{target_key}_mask"], dim=[1, 2]
                    )
                    ...
                    energy_diff = outputs[key] - target["dft_energy"]
                    ...
                    force_diff = torch.norm(outputs[key] - target["dft_forces"], dim=1)
    ...
    def forward(self, batch, H):
        ...
        if self.use_efs_head:
            output = self.efs_head(batch, output)
    ...
    def _predict_step(self, batch, batch_idx, log_outputs=True):
        ...
        if self.qh9:
            ...
            for i in range(self.cur_batch_size):
                ...
                if self.use_efs_head:
                    pred["energy"] = sample["energy"][i].cpu()
                    pred["forces"] = sample["forces"][i].cpu().view(-1, 3)
                ...
                if self._batch_has_ground_truth_hamiltonian(batch):
                    ...
                    if self.use_efs_head:
                        gt["dft_energy"] = batch_one[i].dft_energy.cpu()
                        gt["dft_forces"] = batch_one[i].dft_forces.cpu()
        else:
            sample, traj, _pred = self.sample(batch_one, num_timesteps=self.num_ode_steps_test)
            for i in range(self.cur_batch_size):
        ...
        if self._batch_has_ground_truth_hamiltonian(batch_one) and log_outputs:
            metrics = self.metric(sample, batch_one, self.metric_weights)
    ...
    def sample_qh9(
        ...
        for idx, next_t in enumerate(lin_t[1:]):
            ...
            if self.use_full_target:
                target_full_H = outputs["hamiltonian_full_diagonal_blocks"]
                cur_full_H = batch.init_full_ham_t
                vector_field_full = (target_full_H - cur_full_H) / (1 - cur_t)
                ham_t_full = cur_full_H + vector_field_full * dt.reshape(-1, 1, 1)
            ...
            if self.use_full_target:
                predictions.append(
                    {
                        "hamiltonian_full_diagonal_blocks": outputs[
                            "hamiltonian_full_diagonal_blocks"
                        ].cpu(),
                        "hamiltonian_full_non_diagonal_blocks": outputs[
                            "hamiltonian_full_non_diagonal_blocks"
                        ].cpu(),
                    }
                )
            ...
            batch.init_ham_t = ham_t
            if self.use_full_target:
                batch.init_ham_full_t = ham_t_full
        ...
        if self.use_full_target:
            res_outputs["hamiltonian_full_diagonal_blocks"] = ham_t_full
            res_outputs["hamiltonian_full_non_diagonal_blocks"] = outputs["hamiltonian_full_non_diagonal_blocks"]
    ...
    def _log_sample_metric_qh9_mul(self, batch_one, prefix, num_timesteps=1, post_fix="", mul=5):
        ...
        try:
            ...
            for i in range(mul):
                sample, traj, pred = self.sample(batch_one, num_timesteps=num_timesteps)
                error_dicts = self._metric_qh9(sample, batch_one, metric_weights=self.metric_weights)
    ...
    def _log_inference_error(
        ...
        results = self.metric(sample, batch_one, self.metric_weights)
    ...
    # ==========================================
    # Dataset Testing Methods (Deprecated)
    # ==========================================

    @torch.no_grad()
    def test_over_dataset(self, test_data_loader, default_type):
        """
        Comprehensive evaluation over entire test dataset.
        
        Automatically detects dataset type and uses appropriate evaluation method.
        
        Args:
            test_data_loader: DataLoader for test data
            default_type: Default tensor type for computation
            
        Returns:
            Tuple of (aggregated_errors, trajectory_samples)
        """
        if self.qh9:
            return self.test_over_dataset_qh9(test_data_loader, default_type)
        else:
            return self.test_over_dataset_md17(test_data_loader, default_type)

    @torch.no_grad()
    def test_over_dataset_md17(self, test_data_loader, default_type):
        """
        Test evaluation for MD17 dataset with full matrix representation.
        
        Args:
            test_data_loader: DataLoader for test data
            default_type: Default tensor type
            
        Returns:
            Tuple of (total_error_dict, last_trajectory)
        """
        self.eval()
        total_error_dict = {"total_items": 0}
        loss_weights = DEFAULT_LOSS_WEIGHTS_MD17.copy()
        total_time = 0
        total_graph = 0
        last_traj = []
        ...
        for idx, batch in tqdm(enumerate(test_data_loader)):
            ...
            # Truncate to occupied orbitals only
    ...
    def test_over_dataset_qh9(self, test_data_loader, default_type):
        """
        Test evaluation for QH9 dataset with block-diagonal structure.
        ...
        self.eval()
        total_error_dict = {"total_items": 0}
        loss_weights = DEFAULT_LOSS_WEIGHTS_QH9.copy()
        total_time = 0
        total_graph = 0
        last_traj = []
        ...
        for idx, batch in tqdm(enumerate(test_data_loader)):
            ...
            # Also get direct model output for comparison
            outputs = self(batch)
            ...
            # Transform to appropriate precision and format
            ...
            # Prepare block outputs for evaluation
