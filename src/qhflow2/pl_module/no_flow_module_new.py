...
class LitModel_flow(LitModel):
    ...
    def _corrupt_md17(self, batch, batch_t):
        ...
        Non-flow variant: draw random Hamiltonians as model inputs without
        time interpolation.
        ...
        Returns:
            Corrupted batch with random Hamiltonians used as inputs
        """
        # For the non-flow setup we keep a fixed t=1 to avoid zero-time
        # updates while still carrying the attribute for loss scaling.
        batch.t = torch.ones(
            batch.hamiltonian.shape[0],
            device=batch.hamiltonian.device,
            dtype=batch.hamiltonian.dtype,
        )
        ...
        # Use pure random Hamiltonian as the model input (no time interpolation)
    ...
    def _corrupt_qh9(self, batch, batch_t):
        ...
        Returns:
            Corrupted batch with random diagonal Hamiltonians as inputs
        """
        batch.t = torch.ones(
            batch["diagonal_hamiltonian"].shape[0],
            device=batch["diagonal_hamiltonian"].device,
            dtype=batch["diagonal_hamiltonian"].dtype,
        )
        ...
        batch.random_ham = random_ham
        batch.target_ham = target_ham
        # Directly feed random Hamiltonian as model input (no interpolation)
        batch.init_ham_t = random_ham
        return batch
    ...
    def corrupt(self, batch, mul=1):
        ...
        # Use fixed t=1 for non-flow training; keep attribute for loss scaling
        if self.qh9:
            num_ham = batch["diagonal_hamiltonian"].shape[0]
            t_dtype = batch["diagonal_hamiltonian"].dtype
        else:
            num_ham = batch.num_graphs
            t_dtype = batch.hamiltonian.dtype
        batch_t = torch.ones(num_ham, device=batch.atoms.device, dtype=t_dtype)
    ...
    def corrupt_mul(self, batch):
        ...
        Non-flow variant: duplicates the batch and applies the same random
        corruption with fixed t=1 (kept for interface compatibility).
        ...
        # Fixed t=1; we no longer mix time bins for corruption
        batch_t = torch.ones(num_ham, device=batch.atoms.device, dtype=t_dtype)
    ...
    def sample(
        ...
        """
        Generate a Hamiltonian prediction from a random initial matrix.
        
        Non-flow version: performs a single forward pass from noise (or the
        provided init) instead of integrating an ODE trajectory.
    ...
    def sample_md17(
        ...
        Non-flow version: generate a random Hamiltonian and run a single
        forward pass to predict the target Hamiltonian.
        ...
        batch_size = batch.init_ham.shape[0]

        # Keep t attribute non-zero for downstream weighting logic
        batch.t = torch.ones(batch_size, device=device, dtype=batch.init_ham.dtype)

        # Initialize Hamiltonian input
        ...
        if sample_random:
            ...
            batch.init_ham_t = batch.init_ham.clone()

        # Single forward pass prediction
        outputs = self(batch, batch.init_ham_t)

        # Store simple trajectories for compatibility with logging
        hamiltonian_traj = [batch.init_ham_t.detach().cpu()]
        predictions = [outputs["hamiltonian"].detach().cpu()]

        if self.use_res_target:
            outputs = {"hamiltonian": outputs["hamiltonian"] + batch.init_ham}

        return outputs, hamiltonian_traj, predictions
    ...
    def sample_qh9(
        ...
        Non-flow version: run a single prediction from a random Hamiltonian
        (block-diagonal form).
        ...
        if sample_random:
            ...
            batch.init_ham_t = batch.init_ham.clone()
            if self.use_full_target:
                batch.init_ham_full_t = batch.init_ham_t.clone()

        # Single forward pass prediction
        outputs = self(batch, batch.init_ham_t)

        # Store trajectories for compatibility
        hamiltonian_traj = [batch.init_ham_t.cpu()]
        if self.qh9:
            predictions = [
                {
                    "hamiltonian_diagonal_blocks": outputs["hamiltonian_diagonal_blocks"].detach().cpu(),
                    "hamiltonian_non_diagonal_blocks": outputs["hamiltonian_non_diagonal_blocks"].detach().cpu(),
                }
            ]
        else:
            predictions = [outputs["hamiltonian"].detach().cpu()]

        # Prepare outputs with residual if needed
        if self.qh9:
            diag_out = outputs["hamiltonian_diagonal_blocks"]
            if self.use_res_target:
                diag_out = diag_out + batch["diagonal_init_ham"]
            res_outputs = {
                "hamiltonian_diagonal_blocks": diag_out,
                "hamiltonian_non_diagonal_blocks": outputs["hamiltonian_non_diagonal_blocks"],
            }
            if self.use_non_diagonal_hamiltonian_scale:
                res_outputs["hamiltonian_non_diagonal_blocks"] = (
                    res_outputs["hamiltonian_non_diagonal_blocks"] / self.non_diagonal_hamiltonian_scale
                )
        else:
            ham_out = outputs["hamiltonian"]
            if self.use_res_target:
                ham_out = ham_out + batch.init_ham
            res_outputs = {"hamiltonian": ham_out}
