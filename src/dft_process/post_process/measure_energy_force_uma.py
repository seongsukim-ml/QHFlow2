...
def get_file_idx(file_path):
    return int(file_path.split("_")[-1].split(".")[0]), int(file_path.split("_")[-2])

def process_single_file(args):
    """Process a single file tuple (pred_path, gt_path, calc_path)"""
    pred_path, gt_path, calc_path, omol_path = args
    pred_path_idx = get_file_idx(pred_path)
    gt_path_idx = get_file_idx(gt_path)
    calc_path_idx = get_file_idx(calc_path)
    omol_path_idx = get_file_idx(omol_path)
    if pred_path_idx != gt_path_idx or pred_path_idx != calc_path_idx or pred_path_idx != omol_path_idx:
        return {"error": f"File indices do not match: {pred_path_idx}, {gt_path_idx}, {calc_path_idx}, {omol_path_idx}"}

    pred_path_processed = pred_path.replace(".pt", "_processed.pt")
    gt_path_processed = gt_path.replace(".pt", "_processed.pt")    
