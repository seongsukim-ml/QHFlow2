...
def process_single_file(args):
    """Process a single file tuple (index, pred_path, gt_path, calc_path, omol_path)"""
    data_index, pred_path, gt_path, calc_path, omol_path = args
    ...
    try:
        ...
        return {"error": error_msg, "file": omol_path, "pred_index": data_index, "pred_path": pred_path}
...
def process_batch_stable(file_batch, num_workers=4, timeout=300):
    ...
    successful_results.sort(key=lambda r: r.get("pred_index", float("inf")))
    ...
    if error_results:
        ...
        for result in error_results[:5]:  # Show first 5 failures
            logger.info(
                "    index=%s | file=%s | path=%s | error=%s",
                result.get("pred_index", "Unknown"),
                result.get("file", "Unknown"),
                result.get("pred_path", "Unknown"),
...
def process_sequential_safe(file_tuples, desc="Processing files"):
    ...
    for i, file_tuple in enumerate(tqdm(file_tuples, desc=desc)):
        data_index, pred_path, gt_path, calc_path, omol_path = file_tuple
        try:
            ...
            if result and "error" not in result:
                ...
                logger.error(
                    "  Skipping file due to error | index=%s | file=%s | path=%s | error=%s",
                    result.get("pred_index", "Unknown"),
                    result.get("file", "Unknown"),
                    result.get("pred_path", "Unknown"),
                    result.get("error", "Unknown error"),
            ...
            logger.error(
                "  Error processing file %s/%s | index=%s | pred_path=%s | error=%s",
                i + 1,
                len(file_tuples),
                data_index,
                pred_path,
...
def main(
    ...
    # Create file tuples with original data indices
    file_tuples = [
        (idx, pred_path, gt_path, calc_path, omol_path)
        for idx, (pred_path, gt_path, calc_path, omol_path) in enumerate(
            zip(sliced_pred_list, sliced_gt_list, sliced_calc_list, sliced_omol_list),
            start=start_index,
        )
    ]
    ...
    if os.path.exists(output_path):
        ...
        if "pred_index" not in results_df.columns:
            results_df["pred_index"] = range(start_index, start_index + len(results_df))
        results_df["pred_index"] = results_df["pred_index"].astype(int)

        index_to_pred_path = {idx: pred_path for idx, pred_path, _, _, _ in file_tuples}
        results_df["pred_path"] = results_df["pred_index"].map(index_to_pred_path)
        results_df["pred_file_name"] = results_df["pred_path"].map(lambda path: os.path.basename(path) if isinstance(path, str) else None)

        results_df = results_df.sort_values(by="pred_index").reset_index(drop=True)
    ...
    if results_list:
        ...
        if valid_results:
            ...
            if "pred_index" in results.columns:
                results["pred_index"] = results["pred_index"].astype(int)
                results = results.sort_values(by="pred_index").reset_index(drop=True)
