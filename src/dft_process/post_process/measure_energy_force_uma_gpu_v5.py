...
_pos_unit = "auto"   # Position unit: "auto", "bohr", or "ang"
...
def set_calculation_settings(pred_xc="b3lyp", gt_xc="b3lyp", calc_xc="pbe", basis="def2svp", pred_tol=1e-8, gt_tol=1e-8, pad_eigval=1, filtering=False, pos_unit="auto"):
    """Set global calculation settings for worker processes"""
    global _pred_xc, _gt_xc, _calc_xc, _basis, _pred_tol, _gt_tol, _pad_eigval, _enable_filtering, _pos_unit
    _pred_xc = pred_xc
    _gt_xc = gt_xc
    _calc_xc = calc_xc
    _basis = basis
    ...
    _pos_unit = pos_unit.lower() if pos_unit else "auto"
...
def init_worker(pred_xc, gt_xc, calc_xc, basis, pred_tol, gt_tol, pad_eigval, filtering, pos_unit):
...
def load_or_compute_calc_data(calc_path, gt_path, atoms, pos, force_compute=False):
    """Load existing calc data or compute it using PySCF with optimizations"""
    if os.path.exists(calc_path) and not force_compute:
        try:
    ...
    try:
        ...
        if os.path.exists(gt_calc_path):
            ...
            if "atoms" in gt_calc_data and "pos" in gt_calc_data and "convention" in gt_calc_data:
                ...
                pos = to_numpy_float64(gt_calc_data["pos"])
                if _pos_unit == "bohr":
                    pos = pos * np.float64(BOHR2ANG)
                elif _pos_unit == "ang":
                    pos = pos
                elif _pos_unit == "auto":
                    length_unit = gt_calc_data.get("length_unit", "angstrom").lower()
                    if length_unit == "bohr":
                        pos = pos * np.float64(BOHR2ANG)
                    elif length_unit != "angstrom":
                        raise ValueError(f"Invalid length unit: {gt_calc_data.get('length_unit')}")
                else:
                    raise ValueError(f"Invalid position unit: {_pos_unit}")
...
def process_all_files(file_list, pred_xc, gt_xc, calc_xc, basis, num_workers=4, chunksize=1, maxtasksperchild=1000, output_path=None, pred_tol=1e-8, gt_tol=1e-8, pad_eigval=1, filtering=False, pos_unit="auto"):
    """Process all files using multiprocessing Pool with imap (preserves order)
    ...
    Note: Uses imap instead of imap_unordered to preserve input order
    ...
    with ctx.Pool(
        ...
        initargs=(pred_xc, gt_xc, calc_xc, basis, pred_tol, gt_tol, pad_eigval, filtering, pos_unit),
        maxtasksperchild=maxtasksperchild
    ) as pool:
        try:
            # Use imap to preserve input order (like final_processing_md17.py)
            for idx, result in enumerate(tqdm(
                pool.imap(process_single_file, file_list, chunksize=chunksize),
                total=len(file_list),
    ...
    # Filter results (order is already preserved by imap, but sort by file_index for safety)
    successful_results = [r for r in results if r and "error" not in r]
    # Results are already in order, but sort by file_index to ensure consistency
...
def main(
    ...
    calc_dir_name = f"_calc_{calc_xc}_gpu"
    gt_calc_dir_name = f"_gt_calc_{gt_xc}_gpu"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pos_unit", type=str, default="auto", choices=["auto", "bohr", "ang"],
                       help="Position unit for inputs (auto=use file metadata, bohr=convert to angstrom, ang=leave as angstrom)")
