...
def set_calculation_settings(pred_xc="b3lyp", gt_xc="b3lyp", calc_xc="pbe", basis="def2svp", pred_tol=1e-8, gt_tol=1e-8, pad_eigval=1):
    """Set global calculation settings for worker processes"""
    global _pred_xc, _gt_xc, _calc_xc, _basis, _pred_tol, _gt_tol, _pad_eigval
    _pred_xc = pred_xc
    _gt_xc = gt_xc
    _calc_xc = calc_xc
    _basis = basis
...
def _prepare_measure_directory(base_model_path: str, start_frac: float, end_frac: float, indices: List[int] = None, tol: float = 1e-8) -> Tuple[str, str]:
    ...
    if indices:
        dir_name = f"measure_{_format_indices_indicator(indices)}_adaptive_tol{tol_str}"
    elif _is_full_fraction_range(start_frac, end_frac):
        dir_name = f"measure_full_adaptive_tol{tol_str}"
    else:
        indicator = f"{_format_fraction_label(start_frac)}_{_format_fraction_label(end_frac)}"
        dir_name = f"measure_{indicator}_adaptive_tol{tol_str}"
...
def init_worker(pred_xc, gt_xc, calc_xc, basis, pred_tol, gt_tol, pad_eigval):
...
def compute_energy_forces_from_hamiltonian(hamiltonian, atoms, calc_overlap, calc_mf, grad_frame, convention=None, tol=1e-8, pad_eigval=1):
...
def process_single_file(args: Tuple) -> Dict:
    ...
    pred_processed_suffix = f"_processed_{_pred_xc}_adaptive_tol{tol_str}_pad{pad_eigval_str}"
    ...
    try:
        ...
        with Timer("PRED processing"):
            ...
            if has_pred_calc_forces and has_pred_mo and has_pred_ham:
                ...
                _, _, pred_mo_energy_safe, pred_ham_transformed_safe = compute_energy_forces_from_hamiltonian(
                    pred_data["pred_hamiltonian"], atoms, calc_overlap, calc_mf, grad_frame, convention, tol=_gt_tol, pad_eigval=None
                )                
                _, _, pred_mo_energy_unsatable, pred_ham_transformed_unsatable = compute_energy_forces_from_hamiltonian(
                    pred_data["pred_hamiltonian"], atoms, calc_overlap, calc_mf, grad_frame, convention, tol=_pred_tol, pad_eigval=_pad_eigval
                )
                n_occ = atoms.sum() // 2
                ...
                if max(abs(mo_energy_diff[:n_occ+1])) > 0.03:
                    pred_mo_energy = pred_mo_energy_unsatable
                    pred_ham_transformed = pred_ham_transformed_unsatable
                    print("Index:", file_index, "Unstable")
                else:
                    pred_mo_energy = pred_mo_energy_safe
                    pred_ham_transformed = pred_ham_transformed_safe
                ...
                pred_energy, pred_forces, pred_mo_energy, pred_ham_transformed = compute_energy_forces_from_hamiltonian(
                    pred_data["pred_hamiltonian"], atoms, calc_overlap, calc_mf, grad_frame, convention, tol=_pred_tol
                )
                pred_data.update({
...
def process_all_files(file_list, pred_xc, gt_xc, calc_xc, basis, num_workers=4, chunksize=1, maxtasksperchild=1000, output_path=None, pred_tol=1e-8, gt_tol=1e-8, pad_eigval=1):
    ...
    with ctx.Pool(
        ...
        initargs=(pred_xc, gt_xc, calc_xc, basis, pred_tol, gt_tol, pad_eigval),
...
def main(
    ...
    set_calculation_settings(pred_xc=pred_xc, gt_xc=gt_xc, calc_xc=calc_xc, basis=basis, pred_tol=pred_tol, gt_tol=gt_tol, pad_eigval=pad_eigval)
    ...
    pred_processed_suffix = f"_processed_{pred_xc}_adaptive_tol{tol_str}_pad{pad_eigval_str}"
    ...
    measure_dir_path, measure_dir_name = _prepare_measure_directory(
        model_path_cls.model_path, start_frac, end_frac, indices=indices_for_dir, tol=pred_tol
    )
