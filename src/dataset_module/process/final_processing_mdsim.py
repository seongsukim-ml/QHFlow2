...
Usage:
    ...
    python final_processing.py --molecule aspirin

    # Custom settings
    python final_processing.py --molecule benzene --num_samples 10000 --seed 123
    python final_processing.py --molecule ethanol --grid_level 3 --n_processes 16
    OMP_NUM_THREADS=4 MKL_NUM_THREADS=4 OPENBLAS_NUM_THREADS=4 python final_processing.py --molecule naphthalene --n_processes 4
    OMP_NUM_THREADS=4 MKL_NUM_THREADS=4 OPENBLAS_NUM_THREADS=4 python final_processing.py --molecule salicylic --n_processes 4

    OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 python final_processing.py --molecule salicylic   --n_processes 63 &&
    OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 python final_processing.py --molecule naphthalene --n_processes 63 &&
    OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 python final_processing.py --molecule ethanol     --n_processes 63 &&
    OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 python final_processing.py --molecule aspirin     --n_processes 63

    # Sequential sampling (not random)
    python final_processing.py --molecule toluene --sequential
"""
...
# Parse arguments at module level for now
...
if args.output is None:
    args.output = f"final_{args.molecule}_conv{args.conv_tol:.0e}_grid{args.grid_level}_n{args.num_samples}.h5"
...
def calculate_single_sample(idx, z, pos, energy_ref, force_ref, settings):
    ...
    """
    # Debug: Uncomment to see when each sample starts
    # print(f"[Process {os.getpid()}] Starting sample {idx}", flush=True)
    ...
    try:
        ...
        ovlp = mf.get_ovlp()
        h_core = mf.get_hcore() # Fixed
        
        init_dm = mf.get_init_guess(key='minao') # Fixed
        init_ham = mf.get_fock(dm=init_dm)

        # Run SCF
        # print(f"Running SCF for sample {idx}")
        E_dft = mf.kernel()
        # print(f"SCF for sample {E_dft} completed")
        ...
        # mo_energy and mo_coeff are available after kernel()
        orbital_energies = mf.mo_energy
        orbital_coefficients = mf.mo_coeff

        # Calculate forces
        grad_obj = mf.nuc_grad_method()
        # print(f"Calculating forces for sample {idx}")s
        forces = -grad_obj.kernel()  # Hartree/Bohr
        # print(f"Forces for sample {forces} completed")
...
if args.n_processes == 1:
    ...
    # Use imap_unordered for faster response (order doesn't matter for results collection)
