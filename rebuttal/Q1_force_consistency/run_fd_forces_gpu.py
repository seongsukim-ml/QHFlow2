...
def energy_from_H(positions, atoms_z, H, basis="def2-svp", xc="pbe"):
    """Compute total DFT energy from H matrix via gpu4pyscf."""
    ...
    import cupy as cp
    D_gpu = cp.asarray(D)
    energy = mf.energy_tot(D_gpu)
    return float(energy), mf, energies, mo_coeff


def forces_analytical(mf, mo_energy, mo_coeff):
    """Compute analytical forces (Hellmann-Feynman + Pulay) via gpu4pyscf."""
    import cupy as cp
